...
MEs0 <- moduleEigengenes(data_matrix, moduleColors)$eigengenes
MEs <- orderMEs(MEs0)
#build the sample of diag
datTraits <- diag(8)
datTraits <- as.data.frame(datTraits)
rownames(datTraits) <- rownames(data_matrix)
colnames(datTraits) <- rownames(data_matrix)
nSamples = nrow(data_matrix)
...
datME <- moduleEigengenes(data_matrix, moduleColors)$eigengenes
...
plotMat(t(scale(data_matrix[,moduleColors==which.module ]) ),
...
datKME=signedKME(data_matrix, datME, outputColumnName="MM.")
...
write.csv(t(data_matrix),file = "expression.csv",quote = F)


#1. 可视化全部基因网络
# Calculate topological overlap anew: this could be done more efficiently by saving the TOM
# calculated during module detection, but let us do it again here.
dissTOM = 1-TOMsimilarityFromExpr(data_matrix, power = 24);
...
TOM = TOMsimilarityFromExpr(data_matrix, power = 24);
# Read in the annotation file
# annot = read.csv(file = "GeneAnnotation.csv");
# Select modules需要修改，选择需要导出的模块颜色
modules = c("magenta");
# Select module probes选择模块探测
probes = colnames(data_matrix)
